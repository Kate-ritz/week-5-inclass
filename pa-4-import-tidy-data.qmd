...
youth <- read_csv("data-raw/YRBS2015.csv", skip = 1, na = "N/A",
                  col_types = list(
                    AgeCat = col_number(),
                    Grade = col_number(),
                    Perceptionofweight = col_factor(levels = c("Very underweight","Slightly underweight", "About the right weight", "Slightly overweight", "Very overweight" )),
        `Gender` = "c",
        `Height in meters` = col_number(),
        `Weight in kilograms`= "n",
        `Body Mass Index` = "n",
        `BMI percentile` = "n",
        `Fruit eating` = "c",
        `Salad eating` = "c",
        `Other vegetable eating` = "c",
        `Soda drinking` = "c",
        `Breakfast eating` = "n",
        `Physical activity >= 5 days` = "n",
        `Television watching` = "c",
...
> Very large. Needed to update the file path in order to be able to read it in. The first row of data is the description, which makes the variable names read in as data.
...
youth_clean <- write_csv(youth, file = "data-clean/youth_clean.csv")
```

Why do we keep the code for the data cleaning but set the code chunks to `eval: false`?

> We do this to make the program more efficient and to not waste resources on cleaning the data everytime we want to try and run the code.
...
> Difficulty concentrating and how well they speak English are the most common variables to be missing in the data. Largest number of drinks, oral health care, sexual identity, how well speak English, and difficulty concentrating are the most common to be missing together.

How does the missingness of the data impact what variables you might choose to analyze? Why?

> We are less likely to analyze variables that are commonly missing because certain demographics might be unaccounted for in our information, leading to strewed data compared to actual numbers.
